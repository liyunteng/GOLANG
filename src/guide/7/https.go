...
func rootHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Length", fmt.Sprintf("%d", len(RESPONSE_TEMPLATE)))
...
func YourLoadX509KeyPair(certFile string, keyFile string) (cert tls.Certificate, err error) {
	...
	if keyDERBlock == nil {
		err = errors.New("crypt/tls: failed to parse key PEM data")
		return
	}
...
func main() {
	...
	err := YourListenAndServerTLS(fmt.Sprintf(":%d", SERVER_PORT), "ssl/server.crt", "ssl/server/server.key", nil)
	if err != nil {
